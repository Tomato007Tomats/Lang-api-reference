...
![PyPI - Version](https://pypi.org/project/langchain-text-splitters/#history)![PyPI - License](https://opensource.org/licenses/MIT)![PyPI - Downloads](https://pypistats.org/packages/langchain-text-splitters)
...
#### ``from\_huggingface\_tokenizer`classmethod`¶
...
#### ``from\_tiktoken\_encoder`classmethod`¶
...
#### ``chunk\_overlap`instance-attribute`¶
...
#### ``tokens\_per\_chunk`instance-attribute`¶
...
#### ``get\_separators\_for\_language`staticmethod`¶
...
def custom_iframe_extractor(iframe_tag):
    ...
    Converts the iframe to a Markdown-like link: iframe:<src>.
...
| `preserve_links` | Converts `a` tags to Markdown links (`text`).<br>**TYPE:**`bool`**DEFAULT:**`False` |
| `preserve_images` | Converts `img` tags to Markdown images (`!alt`).<br>**TYPE:**`bool`**DEFAULT:**`False` |
| `preserve_videos` | Converts `video` tags to Markdown<br>video links (`!video`).<br>**TYPE:**`bool`**DEFAULT:**`False` |
| `preserve_audio` | Converts `audio` tags to Markdown<br>audio links (`!audio`).<br>**TYPE:**`bool`**DEFAULT:**`False` |
...
#### ``max\_chunk\_size`class-attribute``instance-attribute`¶
...
#### ``min\_chunk\_size`class-attribute``instance-attribute`¶
