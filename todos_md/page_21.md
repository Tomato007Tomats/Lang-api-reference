...
## ``langchain\_tests.integration\_tests.RetrieversIntegrationTests ¶
...
### ``retriever\_constructor`abstractmethod``property`¶
...
### ``retriever\_constructor\_params`property`¶
...
### ``retriever\_query\_example`abstractmethod``property`¶
...
### ``num\_results\_arg\_name`property`¶
...
### ``test\_ainvoke\_returns\_documents`async`¶
