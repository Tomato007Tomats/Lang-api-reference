...
## ``langchain\_core.document\_loaders ¶
...
| `content_key` | The inputs key to set as Document page content. `'.'` characters<br>are interpreted as nested keys. E.g. `content_key="first.second"` will<br>result in<br>`Document(page_content=format_content(example.inputs"first"))`<br>**TYPE:**`str`**DEFAULT:**`''` |
