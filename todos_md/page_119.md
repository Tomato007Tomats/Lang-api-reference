...
## ``langchain\_core.vectorstores.base.VectorStore ¶
...
| `search_type` | Type of search to perform. Can be `'similarity'`, `'mmr'`, or<br>`'similarity_score_threshold'`.<br>**TYPE:**`str` |
...
### ``similarity\_search`abstractmethod`¶
...
### ``asimilarity\_search\_with\_score`async`¶
...
| `query` | Input text.<br>**TYPE:**`str` |
| `k` | Number of `Document` objects to return.<br>**TYPE:**`int`**DEFAULT:**`4` |
| `**kwargs` | kwargs to be passed to similarity search. Should include<br>`score_threshold`, An optional floating point value between `0` to `1`<br>to filter the resulting set of retrieved docs<br>**TYPE:**`Any`**DEFAULT:**`{}` |

| RETURNS | DESCRIPTION |
| --- | --- |
| `list[tuple[Document, float]]` | List of tuples of `(doc, similarity_score)`. |

### ``asimilarity\_search\_with\_relevance\_scores`async`¶
...
### ``asimilarity\_search\_by\_vector`async`¶
...
### ``amax\_marginal\_relevance\_search`async`¶
...
### ``amax\_marginal\_relevance\_search\_by\_vector`async`¶
...
### ``afrom\_documents`async``classmethod`¶
...
### ``from\_texts`abstractmethod``classmethod`¶
...
### ``afrom\_texts`async``classmethod`¶
...
## ``langchain\_core.vectorstores.base.VectorStoreRetriever ¶
...
### ``search\_type`class-attribute``instance-attribute`¶
...
### ``search\_kwargs`class-attribute``instance-attribute`¶
...
### ``validate\_search\_type`classmethod`¶
...
## ``langchain\_core.vectorstores.in\_memory.InMemoryVectorStore ¶
