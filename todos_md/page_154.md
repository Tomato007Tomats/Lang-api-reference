...
## ``langchain\_classic.runnables.hub ¶
...
| METHOD | DESCRIPTION |
| --- | --- |
| `__init__` | Initialize the `HubRunnable`. |
...
#### ``config\_factories`class-attribute``instance-attribute`¶
...
#### ``custom\_input\_type`class-attribute``instance-attribute`¶
...
#### ``custom\_output\_type`class-attribute``instance-attribute`¶
...
| `api_url` | The URL of the LangChain Hub API.<br>Defaults to the hosted API service if you have an api key set,<br>or a localhost instance if not.<br>**TYPE:**`str | None`**DEFAULT:**`None` |
| `api_key` | The API key to use to authenticate with the LangChain Hub API.<br>**TYPE:**`str | None`**DEFAULT:**`None` |
| `**kwargs` | Additional keyword arguments to pass to the parent class.<br>**TYPE:**`Any`**DEFAULT:**`{}` |
...
## ``langchain\_classic.runnables.openai\_functions ¶
...
Bases: `RunnableBindingBase[BaseMessage, Any]`

A runnable that routes to the selected function.

| METHOD | DESCRIPTION |
| --- | --- |
| `__init__` | Initialize the `OpenAIFunctionsRouter`. |
| `get_name` | Get the name of the `Runnable`. |
| `get_input_schema` | Get a Pydantic model that can be used to validate input to the `Runnable`. |
| `get_input_jsonschema` | Get a JSON schema that represents the input to the `Runnable`. |
| `get_output_schema` | Get a Pydantic model that can be used to validate output to the `Runnable`. |
...
```
__init__(
    runnables: Mapping[str, Runnable[dict, Any] | Callable[[dict], Any]],
...
| PARAMETER | DESCRIPTION |
| --- | --- |
| `runnables` | A mapping of function names to runnables.<br>**TYPE:**`Mapping[str, Runnable[dict, Any] | Callable[[dict], Any]]` |
| `functions` | Optional list of functions to check against the runnables.<br>**TYPE:**`list[OpenAIFunction] | None`**DEFAULT:**`None` |
